...
from models.model_porpoise import PorpoiseMMF, PorpoiseAMIL, TMIL  # , PorpoiseMMF_Fast
...
class EarlyStopping:
    ...
    def __init__(self, warmup=5, patience=5, stop_epoch=20, verbose=False):
        """
        Args:
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.task_type == 'multi_task':    
        cls_loss = FocalLoss(class_num=train_split.stage_num)
        ...
        raise NotImplementedError
    # modal_loss = torch.nn.NLLLoss()
    if args.alignment:
        modal_loss = torch.nn.CrossEntropyLoss()
    else:
        modal_loss = None
    ...
    if args.model_type == 'porpoise_mmf':
        ...
        model_dict = {'omic_input_dim': args.omic_input_dim, 'model_size_omic': args.model_size_omic,
                      'n_classes': args.n_classes, 'stage_num': train_split.stage_num}
        model = SNN(**model_dict)
    elif args.model_type == 'deepset':
        model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'n_classes': args.n_classes}
        model = MIL_Sum_FC_surv(**model_dict)
    elif args.model_type == 'amil':
        model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'n_classes': args.n_classes, 'stage_num': train_split.stage_num}
        model = MIL_Attention_FC_surv(**model_dict)
    elif args.model_type == 'mi_fcn':
        model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'num_clusters': 10,
    ...
    elif args.model_type == 'cmaf':
        model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'n_classes': args.n_classes,
                      'gate_path': args.gate_path, 'gate_omic': args.gate_omic, 'scale_dim1': args.scale_dim1,
                      'scale_dim2': args.scale_dim2,
                      'skip': args.skip, 'dropinput': args.dropinput, 'path_input_dim': args.path_input_dim,
                      'use_mlp': args.use_mlp}
        model = CMAF(**model_dict)
    ...
    schedular = torch.optim.lr_scheduler.StepLR(optimizer, step_size=6, gamma=0.5, last_epoch=-1)
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(warmup=0, patience=5, stop_epoch=20, verbose=True)
    else:
        early_stopping = None
    ...
    for epoch in range(args.max_epochs):
        if args.task_type == 'multi_task' or 'survival':
            train_loop_survival(epoch, model, train_loader, optimizer, args.n_classes, args.mode, args.task_type, writer, loss_fn, reg_fn, args.lambda_reg, args.gc, cls_loss=cls_loss, modal_loss=modal_loss)
            stop = validate_survival(cur, epoch, model, val_loader, args.n_classes, args.mode, args.task_type, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir, cls_loss=cls_loss, modal_loss=modal_loss)
    ...
    results_val_dict, val_cindex, stage_acc = summary_survival(model, val_loader, args.n_classes, args.mode, args.task_type)

    print('Val c-Index: {:.4f}'.format(val_cindex))
    writer.close()
    return results_val_dict, val_cindex, stage_acc


def train_loop_survival(epoch, model, loader, optimizer, n_classes, mode, task_type, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16, cls_loss=None, modal_loss=None):
    ...
    # for batch_idx, data in enumerate(loader):
        p = float(batch_idx + epoch * len(loader)) / 30 / len(loader)
        ...
        if mode=='pathomic':
            h, modality_pred, modality_label = model(alpha, x_path=data_WSI, x_omic=data_omic)
            modality_label = modality_label.to(device)
        else:
            h = model(x_path=data_WSI, x_omic=data_omic)
        
        if not isinstance(h, tuple):
            loss_surv = loss_fn(h=h, y=y_disc, t=event_time, c=censor)
            if mode=='pathomic' and modal_loss:
                loss_modality = modal_loss(modality_pred, modality_label)

            else:
                loss_modality = 0
            loss = loss_surv + T * loss_modality # + rec_loss
            loss_value = loss.item()
        else:
            h_path, h_omic, h_mm, none1, none2 = h  # too many values to unpack (expected 3)修改
            print('h_path', h_path)
            print('h_omic', h_omic)
            print('h_mm', h_mm)
        ...
        train_loss_surv += loss_surv.item()
...
def validate_survival(cur, epoch, model, loader, n_classes, mode, task_type, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, cls_loss=None, modal_loss=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor, stage) in enumerate(loader):
        # p = float(batch_idx + epoch * len(loader)) / 35 / len(loader)

        p = float(batch_idx + epoch * len(loader)) / 40 / len(loader)
        alpha = 2. / (1. + np.exp(-10 * p)) - 1
        data_WSI, data_omic = data_WSI.to(device), data_omic.to(device)
        y_disc = y_disc.to(device)
        event_time = event_time.to(device)
        ...
        with torch.no_grad():
            # h, rec_omic, rec_path = model(x_path=data_WSI, x_omic=data_omic)  # return hazards, S, Y_hat, A_raw, results_dict
            if mode=='pathomic':
                h, modality_pred, modality_label= model(alpha, x_path=data_WSI, x_omic=data_omic)
                modality_label = modality_label.to(device)
            else:
        ...
        if not isinstance(h, tuple):
            ...
            # loss += recon_loss
            if mode=='pathomic' and modal_loss:
                loss_modal = modal_loss(modality_pred, modality_label)

            else:
                loss_modal = 0
            loss = loss + loss_modal
            loss_value = loss.item()
        else:
...
def summary_survival(model, loader, n_classes, mode, task_type):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    test_loss = 0.
    stage_acc = 0.
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor, stage) in enumerate(loader):
        ...
        with torch.no_grad():
            if mode=='pathomic':
                h, modal_pred, modal_label = model(alpha=0, x_path=data_WSI, x_omic=data_omic)
            else:
                h = model(x_omic=data_omic, x_path=data_WSI)
    ...
    c_index = concordance_index_censored((1 - all_censorships).astype(bool), all_event_times, all_risk_scores, tied_tol=1e-08)[0]
    return patient_results, c_index, stage_acc
