...
def collate_MIL_survival(batch):
    ...
    stage = torch.LongTensor([item[5].item() for item in batch])

    return [img, omic, label, event_time, c, stage]
...
def l1_reg_omic(model, reg_type=None):
    ...
    if hasattr(model, 'discriminator'):
        l1_reg  = l1_reg + l1_reg_all(model.discriminator) + l1_reg_all(model.fc_omic) 
    elif hasattr(model, 'unimodal_cls'):
        l1_reg = l1_reg + l1_reg_all(model.discriminator) + l1_reg_all(model.fc_omic) + l1_reg_all(model.unimodal_cls)
    elif hasattr(model, 'fc_omic'):
        l1_reg += l1_reg_all(model.fc_omic)
    else:
...
def get_custom_exp_code(args):
    ...
    if args.model_type == 'porpoise_mmf':
      ...
      param_code += 'MCAT'
    elif args.model_type == 'multi_SNN':
      param_code += 'multi_SNN'
    elif args.model_type == 'Multi_MMF':
      param_code += 'Multi_MMF'
