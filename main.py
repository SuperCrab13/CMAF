...
def main(args):
	...
	for i in folds:
		...
		if args.task_type == 'survival':
			val_latest, cindex_latest, stage_acc = train(datasets, i, args)
			latest_val_cindex.append(cindex_latest)

		elif args.task_type == 'multi_task':
			val_latest, cindex_latest, stage_acc = train(datasets, i, args)
			latest_val_cindex.append(cindex_latest)
			latest_stage_acc.append(stage_acc)
	...
	if args.task_type == 'multi_task':
		results_latest_df = pd.DataFrame({'folds': folds, 'val_cindex': latest_val_cindex, 'val_acc': latest_stage_acc})
...
parser.add_argument('--data_root_dir',   type=str, default='/data/run01/scz5319/xly/PORPOISE-master/selec_wsi_pt', help='Data directory to WSI features (extracted via CLAM')
...
parser.add_argument('--split_dir',       type=str, default='tcga_coad', help='Which cancer type within ./splits/<which_splits> to use for training. Used synonymously for "task" (Default: tcga_blca_100)')
...
parser.add_argument('--task_type',		 type=str, default='survival', choices=['multi_task', 'survival'], help='Multi task or survival')
parser.add_argument('--alignment',		 action='store_true', default=False, help='Using  adversarial training to align feature')

### Model Parameters.
parser.add_argument('--model_type',      type=str, default='cmaf', help='Type of model (Default: mcat)')
...
if 'survival' in args.task:
	...
	dataset = Generic_MIL_Survival_Dataset(csv_path = './%s/%s_all_clean.zip' % (args.dataset_path, study),
										   mode = args.mode,
