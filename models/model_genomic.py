...
##########################
class SNN(nn.Module):
    def __init__(self, omic_input_dim: int, model_size_omic: str='big', n_classes: int=4, stage_num=10):
        super(SNN, self).__init__()
        self.n_classes = n_classes
        self.size_dict_omic = {'small': [256, 256, 256, 256], 'big':[2048, 1024, 512, 256] }# [512, 384]}
        
        ### Constructing Genomic SNN
        hidden = self.size_dict_omic[model_size_omic]
        # fc_omic = [SNN_Block(dim1=omic_input_dim, dim2=hidden[0])]
        fc_omic = [MLP_Block(dim1=omic_input_dim, dim2=hidden[0])]
        for i, _ in enumerate(hidden[1:]):
            # fc_omic.append(SNN_Block(dim1=hidden[i], dim2=hidden[i+1], dropout=0.25))
            fc_omic.append(MLP_Block(dim1=hidden[i], dim2=hidden[i+1], dropout=0.25))
