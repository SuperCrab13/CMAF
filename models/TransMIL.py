...
class PPEG(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # cnn_feat = feat_token.transpose(1, 2).view(B, C, H, W) # cls_token
...
class TransMIL(nn.Module):
    def __init__(self, input_dim=384):
        ...
        self.layer1 = MultiheadAttention(num_heads=1, embed_dim=input_dim)
        self.norm = nn.LayerNorm(input_dim)


    def forward(self, h, omic):
        h = h.unsqueeze(0
        ...
        add_length = _H * _W - H
        h = torch.cat([h, torch.zeros_like(h[:, :add_length, :])], dim=1) #[B, N, 512]
        B = h.shape[0]

        omic = omic.view(1, 1, omic.shape[-1])

        h, A_coattn = self.layer1(omic, h, h) #[B, N, 512]
...
def multi_head_attention_forward(
    ...
    attn_output_weights = torch.bmm(q, k.transpose(1, 2)) # attention

    # attn_output_weights = torch.cosine_similarity(q, k, dim=2)
    ...
    # print(attn_output_weights.shape, v.shape)
    attn_output = attn_output_weights.view(attn_output_weights.size()[2], 1)*v
