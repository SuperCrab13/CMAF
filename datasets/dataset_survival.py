...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        if 'kirc' in csv_path:
            self.cancer = 'kirc'
        elif 'gbmlgg' in csv_path:
            self.cancer = 'gbmlgg'
        elif 'brca' in csv_path:
            self.cancer = 'brca'
        elif 'kirp' in csv_path:
            self.cancer = 'kirp'
        ...
        metadata = [ 'disc_label', 'Unnamed: 0', 'case_id', 'label', 'slide_id', 'survival_months', 'stage','censorship']
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str='train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(df_slice, metadata=self.metadata, mode=self.mode, signatures=self.signatures, data_dir=self.data_dir, label_col=self.label_col, patient_dict=self.patient_dict, num_classes=self.num_classes, stage_dict=self.stage_dict, cancer=self.cancer)
        else:
            split = None
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    return (path_features, torch.zeros((1,1)), label, event_time, c, stage)
                    ...
                    #print("cluster,genomic_features",genomic_features)
                    return (path_features, cluster_ids, genomic_features, label, event_time, c, stage)

                elif self.mode == 'omic':
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx])
                    #print("omic,genomic_features",genomic_features)
                    return (torch.zeros((1,1)), genomic_features.unsqueeze(dim=0), label, event_time, c, stage)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, 
        signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2, stage_dict=None, cancer='brca'):
        self.use_h5 = False
        self.slide_data = slide_data
        ...
        print("self.signatures",self.signatures)
        if cancer == 'brca':
            self.genomic_dim = [2854, 2924, 5214, 5380] # brca
        elif cancer == 'kirc':
            self.genomic_dim = [2854, 2924, 5255, 5416] # kirc
        elif cancer == 'gbmlgg':
            self.genomic_dim = [2854, 2924, 5246, 5419] # gbmlgg
        elif cancer == 'kirp':
            self.genomic_dim = [480, 3334, 3354, 3473]
        ...
        if self.signatures is not None:
            ...
            for col in self.signatures.columns:
                ...
                omic = np.concatenate([omic+mode for mode in ['_rnaseq','_mut']])#'_cnv',
                print("2\self.genomic_features",self.genomic_features)
    ...
    def apply_scaler(self, scalers: dict = None):
        ...
        # transformed_omic = np.hstack([scaler_rna, scaler_cnv, scaler_mut]) ### methylation ablation
        transformed = pd.DataFrame(transformed_omic)
        # transformed.columns = self.genomic_features.columns
        # transformed.columns = self.genomic_features.columns[: self.genomic_dim[0]].to_list() + self.genomic_features.columns[self.genomic_dim[0]: self.genomic_dim[1]].to_list() + self.genomic_features.columns[self.genomic_dim[2]: self.genomic_dim[3]].to_list()              
        # transformed_omic = np.hstack([scaler_rna, scaler_mut, scaler_methy])
        # transformed = pd.DataFrame(transformed_omic)
        # transformed.columns = self.genomic_features.columns[:self.genomic_dim[0]].to_list() + self.genomic_features.columns[self.genomic_dim[0]: self.genomic_dim[1]].to_list() + self.genomic_features.columns[self.genomic_dim[1]: self.genomic_dim[2]].to_list()   
